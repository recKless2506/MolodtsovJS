...
window.onload = function(){ 
    ...
    document.getElementById("btn_backspace").onclick = function() {
      if (selectedOperation) {
          // Если выбрана операция, удаляем символ из второго числа (b)
          if (b.length > 0) {
              b = b.slice(0, -1); // Удаляем последний символ
              outputElement.innerHTML = b === '' ? '0' : b; // Если строка пустая, выводим 0
          }
      } else {
          // Если операция не выбрана, удаляем символ из первого числа (a)
          if (a.length > 0) {
              a = a.slice(0, -1); // Удаляем последний символ
              outputElement.innerHTML = a === '' ? '0' : a; // Если строка пустая, выводим 0
          }
      }
    };
...
document.getElementById("btn_op_methane").onclick = function() {
  // Проверяем, есть ли введенное значение (масса метана)
  if (a === '') {
      alert("Введите массу метана (в граммах).");
      return;
  }
  // Молярная масса метана (CH₄)
  const molarMassMethane = 16.04; // г/моль (C: 12.01, H: 1.008 * 4)
  // Уравнение реакции: CH₄ + 2O₂ → CO₂ + 2H₂O
  // Для сгорания 1 моля CH₄ требуется 2 моля O₂
  // Рассчитываем количество молей метана
  const molesMethane = parseFloat(a) / molarMassMethane;
  // Рассчитываем количество молей кислорода
  const molesOxygen = molesMethane * 2;
  // Молярный объем газа при нормальных условиях (0°C и 1 атм)
  const molarVolume = 22.41; // л/моль
  // Рассчитываем объем кислорода
  const volumeOxygen = molesOxygen * molarVolume;
  // Выводим результат
  outputElement.innerHTML = volumeOxygen.toFixed(2);
};
